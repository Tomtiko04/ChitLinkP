...
const AllContacts = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex h-[50vh] flex-grow items-center justify-center sm:h-[80vh]">
        <Icon icon="line-md:loading-twotone-loop" className="h-12 w-12 text-[#cf983a]" />
      </div>
    );
  }
  ...
  return (
    <div
      className={`flex h-full flex-col bg-[#ffffff] p-4 text-white transition-all duration-300 sm:bg-[#f5f4f0] sm:p-0 ${isSelectionActive ? 'pb-24' : ''}`}
    >
      {/* Mobile Card View */}
      ...
      {pageCount > 1 && (
        <div className="my-6 flex items-center justify-between rounded-lg bg-[#f5f4f0] px-6 py-2 sm:rounded-none sm:bg-[#ffffff]">
          ...
          <ReactPaginate
            forcePage={page - 1}
            previousLabel={<Icon icon="ion:chevron-back" color="#000" className="h-5 w-5" />}
            nextLabel={<Icon icon="ion:chevron-right" color="#000" className="h-5 w-5" />}
            breakLabel={'...'}
            pageCount={pageCount}
